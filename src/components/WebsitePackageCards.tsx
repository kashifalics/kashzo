...
import { websiteDiscount, websitePackages } from '@/lib/config/pricing';
...
export function WebsitePackageCards() {
  return <div className={styles.grid}>{websitePackages.map((item) => <article key={item.name} className={`${styles.card} ${item.highlighted ? styles.highlighted : ''}`}>
    <div className={styles.badges}>{item.highlighted && <span className={styles.popular}>MOST POPULAR</span>}<span className={styles.discount}>{websiteDiscount} OFF</span></div>
    ...
    <Link href="/contact" className={styles.cta}>Claim {websiteDiscount} Offer <span aria-hidden="true">→</span></Link>
  </article>)}</div>;
}
