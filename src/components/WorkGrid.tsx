...
export function WorkGrid({ projects }: { projects: Project[] }) {
  ...
  return <>
    ...
    <div className={styles.projectGrid}>{visible.map(project => <Link key={project.id} href={`/work/${project.slug}`} className={styles.projectCard}><div className={styles.projectVisual} data-category={project.category} aria-hidden="true"><i/><i/><i/><strong>{project.category === 'AI' ? 'Intelligence workflow' : project.category === 'Web' ? 'Platform architecture' : project.category === 'Mobile' ? 'Connected product' : 'Growth system'}</strong></div><div className={styles.projectMeta}><span>{project.category}</span><span>{project.industry}</span></div><h2>{project.title}</h2><p>{project.summary}</p><div className={styles.tags}>{project.technologies.slice(0,4).map(technology => <span key={technology}>{technology}</span>)}</div><span className={styles.projectCta}>View Case Study →</span></Link>)}</div>
  </>;
}
