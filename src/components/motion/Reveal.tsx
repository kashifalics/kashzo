...
export function Reveal({ children, className, delay = 0, y = 24 }: RevealProps) {
  ...
  return <motion.div className={className} initial={reduceMotion ? false : { opacity: 0, y }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.18 }} transition={{ duration: 0.65, delay, ease: easing }}>{children}</motion.div>;
}
